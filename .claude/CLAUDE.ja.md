...
### 問題
Claude Code の UI が再度変更され、選択肢プロンプトで数字キー（1、2、3など）による選択が機能しなくなった。ユーザーは上下カーソルキーでナビゲーションする必要が生じた。

### 修正内容
`*ai*` バッファのキーマップに2つのキーバインドを追加:
- `C-c C-n` - tmux に `Down` 矢印キーを送信（次の選択肢へ）
- `C-c C-p` - tmux に `Up` 矢印キーを送信（前の選択肢へ）

**新規関数** (emacs-ai-agent-bridge.el):
- `emacs-ai-agent-bridge-send-down` - 選択中の tmux セッションに `Down` キーを送信
- `emacs-ai-agent-bridge-send-up` - 選択中の tmux セッションに `Up` キーを送信
...
```
 ...
 ❯ 2. Yes, and don't ask again for: git push:*
   3. No
```
→ `C-c C-p` でオプション1に移動し、`C-m` で確定。

**補足**: `C-n` と `C-p` は意図的に使用しない。`*ai*` バッファ内でのカーソル自由移動（tmux スクロールバック履歴の閲覧）に使用するため。

**動作確認**:
- ✓ `C-c C-n` が tmux に `Down` 矢印キーを正しく送信
- ✓ `C-c C-p` が tmux に `Up` 矢印キーを正しく送信
- ✓ 両キーバインドとも選択中の tmux セッションを尊重（Issue #15 のパターンに準拠）

## Issue #21 実装

### 機能要望
`*ai*` バッファのテキスト入力プロンプト（`❯`）上でEnterを押した際に、ミニバッファから入力を受け付けてtmuxに送信する機能。

### 問題
Claude Codeのテキスト入力プロンプト（`❯` マーク付き）に対して、`*ai*` バッファから直接テキストを入力する方法がなかった。

### 実装

**新規関数**:
- `emacs-ai-agent-bridge-is-cursor-on-input-prompt-p` - カーソル位置がテキスト入力プロンプト行かどうかを判定。`❯` を含み、上下の行に `----------` セパレータがあることを条件とする
- `emacs-ai-agent-bridge-flash-region` - 送信テキストを一時的にハイライトするフラッシュ効果
- `emacs-ai-agent-bridge-read-and-send-input` - ミニバッファからテキストを読み取り、tmuxセッションに送信。送信後、プロンプト行に入力テキストを表示

**変更関数**:
- `emacs-ai-agent-bridge-smart-return` - プロンプト検出中かつ入力プロンプト行上の場合、ミニバッファ入力モードを起動するよう拡張

**動作フロー**:
1. `*ai*` バッファで `❯` プロンプト行にカーソルを合わせてEnterを押す
2. ミニバッファに「AI input: 」プロンプトが表示される
3. テキストを入力してEnterで送信
4. 入力テキストがプロンプト行にフラッシュ効果付きで表示される

**プロンプト判定条件**:
- 行に `❯` が含まれている
- 行に数字+ドット（選択肢の番号）が含まれていない
- 上の行に `-----------` が含まれている
- 下の行に `-----------` が含まれている

**動作確認**:
- ✓ `❯` プロンプト行でEnterを押すとミニバッファ入力が起動
- ✓ 入力テキストがtmuxセッションに正しく送信される
- ✓ 送信後、プロンプト行にフラッシュ効果で入力テキストが表示される
- ✓ 選択肢プロンプト（数字付き）では誤動作しない
